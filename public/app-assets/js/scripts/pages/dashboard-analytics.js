...
(function(window, document, $) {
    ...
    ********************************************/
    ...
    function getCountryByCode(object, value) {
        return Object.keys(object).find(key => object[key] === value);
      }

    loadDoc('/data/cases', (data)=>{
        const visitorData={}
        for(let item of data) {
            const country = alpha2[item['Country/Region']]
            const cases = item[Object.keys(item).pop()]; //gets the last value (number of cases) of the object
            if(visitorData[country] === undefined)
                visitorData[country] = parseInt(cases)
            else
                visitorData[country] +=parseInt(cases) 
        };
        ...
        $('#world-map-markers').vectorMap({
            ...
            onRegionTipShow: (e, el, code)=>{
              el.html(el.html()+' (Cases recorded - '+visitorData[code]+')');
            },
            onRegionClick: (e, code)=>{
                let countryName = getCountryByCode(alpha2, code);
                console.log("code: ", code, "Country: ", countryName);
                showGraph(countryName);
                showSummary(countryName);
                console.log(" hey I have clicked --code: ", code, "Country: ", countryName)
            }
          });
      })
